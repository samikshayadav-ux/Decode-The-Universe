...
const LiveLeaderboard = () => {
  ...
  const processData = (data) => {
    ...
    const processedTeams = data.map(team => {
      ...
      const stageTimes = team.stageTimes || { stage1: null, stage2: null, stage3: null };
      const totalTimeSpent = team.totalTimeSpent || 0;
      
      // Only show displayTime if round is completed, otherwise show null
      ...
      return {
        ...
        status: status,
        stage1Time: stageTimes.stage1 || null,
        stage2Time: stageTimes.stage2 || null,
        stage3Time: stageTimes.stage3 || null,
    ...
    const sortedTeams = [...processedTeams].sort((a, b) => {
      ...
      // Then by total time (ascending - faster is better)
      if (a.sortTime !== b.sortTime) return a.sortTime - b.sortTime;

      // Finally by last submission time
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-gray-800/50 backdrop-blur-sm rounded-xl shadow-2xl overflow-hidden border border-gray-700/50">
          <table className="w-full">
            <thead className="bg-gradient-to-r from-green-900/50 to-blue-900/50">
              <tr>
                <th className="p-5 text-left">Rank</th>
                <th className="p-5 text-left">Team Name</th>
                <th className="p-5 text-center">Stage 1</th>
                <th className="p-5 text-center">Stage 2</th>
                <th className="p-5 text-center">Stage 3</th>
                <th className="p-5 text-right">Total Time</th>
              </tr>
            </thead>
                    ...
                    >
                      ...
                      <td className="p-5 text-center font-mono text-gray-300">
                        <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold ${
                          team.stage1Time !== null ? ' text-green-300' : ' text-gray-400'
                        }`}>
                          {team.stage1Time !== null ? formatTime(team.stage1Time) : '--:--'}
                        </span>
                      </td>
                      <td className="p-5 text-center font-mono text-gray-300">
                        <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold ${
                          team.stage2Time !== null ? ' text-blue-300' : ' text-gray-400'
                        }`}>
                          {team.stage2Time !== null ? formatTime(team.stage2Time) : '--:--'}
                        </span>
                      </td>
                      <td className="p-5 text-center font-mono text-gray-300">
                        <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold ${
                          team.totalTime !== null && team.stage1Time !== null && team.stage2Time !== null ? ' text-purple-300' : ' text-gray-400'
                        }`}>
                          {team.totalTime !== null && team.stage1Time !== null && team.stage2Time !== null
                            ? formatTime(team.totalTime - team.stage1Time - team.stage2Time) 
                            : '--:--'}
                        </span>
                      </td>
                      <td className="p-5 text-right font-mono text-gray-300 text-lg font-bold">
                        <span className={`inline-flex items-center px-4 py-2 rounded-full ${
                          team.totalTime !== null 
                            ? ' text-yellow-300' 
                            : ' text-gray-400'
                        }`}>
                          {team.totalTime !== null ? formatTime(team.totalTime) : '--:--'}
                        </span>
                      </td>
                    </motion.tr>
                  );
