...
const Round0Gateway = () => {
  ...
  const correctPassword = 'KLEround0';

  // Store intended round and handle logged-in users
  useEffect(() => {
    if (isLoggedIn && authData) {
      // Only redirect if already logged in for THIS round (round0)
      const userRound = authData.allowedRound;
      const isRound0 = userRound === 'round0' || userRound === 0;
      
      if (isRound0) {
        console.log('User already logged in for round0, redirecting to game');
        navigate('/round0/game', { replace: true });
      }
    }
  ...
  const handleSubmit = (e) => {
    ...
    setTimeout(() => {
      if (password.trim() === correctPassword) {
        ...
        navigate('/auth', { state: { from: { pathname: '/round0' } } });
      } else {
