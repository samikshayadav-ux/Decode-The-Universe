...
const Round2Gateway = () => {
  ...
  const fetchRoundInfo = useCallback(async () => {
    try {
      ...
      if (response.ok) {
        ...
        const round = rounds.find(r => r.roundNumber === 2);
  ...
  const hasAccess = authData?.unlockedRounds?.includes(2);
      ...
      <div className="flex-1 flex flex-col items-start justify-center z-10">
        ...
        >
          <div className="flex items-center gap-4">
            <span className="bg-white text-black px-4 py-1 font-black text-2xl tracking-tighter italic">02</span>
            <div className="h-px w-24 bg-white/20" />
            <div className="text-xs font-black tracking-[0.5em] text-white/30 uppercase">SEQUENCE_PHASE_02</div>
          </div>
        ...
        >
           ...
           <p className="mt-8 text-white/30 text-[10px] md:text-xs max-w-md uppercase leading-relaxed font-bold tracking-widest">
             CRITICAL: CROSS-COMMUNICATION PROTOCOLS ENABLED for PHASE 2. <br />
             LEVEL OF DIFFICULTY: EXTREME. GOOD LUCK.
           </p>
        </motion.div>
      </div>
