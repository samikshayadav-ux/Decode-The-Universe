...
const Home = () => {
  ...
  return (
    <div className="min-h-screen bg-black text-white flex flex-col items-center justify-center relative overflow-hidden font-mono grid-bg p-6">
      ...
      >
        <div className="flex items-center gap-2 bg-accent text-black px-3 py-1 text-sm font-black mb-4">
          <Terminal size={16} />
          <span>ESTABLISHED 2026 // DECODE_SEQUENCE_INIT</span>
        </div>
        
        <h1 className="text-6xl md:text-9xl font-black tracking-tighter leading-[0.85] flex flex-col items-center italic">
          <span className="block">DECODE</span>
          <span className="block text-accent">THE</span>
          <span className="block">UNIVERSE</span>
        </h1>

        <div className="w-full h-2 bg-white/5 mt-8 relative overflow-hidden">
          <motion.div 
            initial={{ x: '-100%' }}
            animate={{ x: '100%' }}
      ...
      <motion.div 
        ...
        className="w-full max-w-5xl z-10 mt-12 bg-black/50 backdrop-blur-sm border-y-2 border-white/10"
      >
        <RoundProgressBar 
      ...
      <motion.div 
        ...
        className="z-10 mt-12 w-full max-w-md"
      >
        <button
          onClick={handleRoundButtonClick}
          disabled={currentRoundNumber === null}
          className={`
            w-full border-4 py-8 px-6 flex items-center justify-between text-2xl font-black uppercase tracking-tighter transition-all duration-150
        ...
        >
          <span>
            {currentRoundNumber !== null 
              ? `ENTER ROUND ${currentRoundNumber === 0 ? 1 : currentRoundNumber === 3 ? 'FINAL' : currentRoundNumber}` 
              : 'ROUND LOCKED'}
          </span>
          {currentRoundNumber !== null ? <ArrowRight size={32} /> : <Lock size={32} />}
        </button>
      </motion.div>
      ...
      <motion.div 
        ...
        className="absolute bottom-8 left-0 w-full px-8 flex flex-col md:flex-row justify-between items-center gap-6 z-10"
      >
        <div className="flex gap-8 order-2 md:order-1">
          {menuItems.map((item) => (
            <button
              ...
              className="text-sm font-black tracking-widest hover:text-accent transition-colors underline-offset-4 hover:underline"
            >
              {item.name}
            </button>
          ))}
        ...
        <div className="flex items-center gap-4 bg-white/5 border-2 border-white/10 px-4 py-2 order-1 md:order-2">
          <div className="flex items-center gap-2">
            <div className={`w-3 h-3 ${isLoaded ? 'bg-green-500 animate-pulse' : 'bg-gray-500'}`} />
            <span className="text-xs font-bold tracking-widest text-white/50 uppercase">System Status: {isLoaded ? 'Online' : 'Connecting'}</span>
          </div>
          <div className="w-px h-4 bg-white/20" />
          <div className="text-xs font-bold tracking-widest text-accent uppercase">
            Team: {authData?.teamName || 'GUEST_01'}
          </div>
        </div>
      </motion.div>
    </div>
